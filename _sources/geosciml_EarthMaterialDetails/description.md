# GeoSciML EarthMaterialDetails

JSON Schema building block for the `EarthMaterialDetails` package of **GeoSciML 4.1**, encoding `«FeatureType»` classes as JSON-FG-compliant features and `«DataType»` / `«CodeList»` / `«Union»` classes per **OGC Best Practice 24-017r1** (*UML to JSON Encoding Rules*).
...
- `../geosciml_GeologyBasic/schema.json#EarthMaterialAbstractDescription`
...
- [Minimal](examples/exampleEarthMaterialDetailsMinimal.json) — bare valid instance.
- [Complete](examples/exampleEarthMaterialDetailsComplete.json) — fully-populated example.

## Source

- UML: `geosciml4.1.xmi`, package `EarthMaterialDetails`.
